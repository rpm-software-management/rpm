	  ...
	  if (data == NULL)
		   ...
		   ++inner)
		{
		  ...
		  if (sym == NULL)
		      ...
		      static const int value[4] =
			{
		      ...
		      if (strcmp (names[inner],
				  elf_strptr (elf, shdr->sh_link,
					      sym->st_name)) != 0)
			{
			  printf ("symbol %d has different name\n", inner);
			  result = 1;
			}

		      if (sym->st_value != value[inner])
			{
			  printf ("symbol %d has wrong value\n", inner);
			  result = 1;
			}

		      if (sym->st_other != 0)
			{
			  printf ("symbol %d has wrong other info\n", inner);
			  result = 1;
			}

		      if (sym->st_shndx != 1)
			{
			  printf ("symbol %d has wrong section reference\n",
				  inner);
			  result = 1;
			}
			...
			{
			  printf ("symbol %d has wrong type or binding\n",
				  inner);
			  result = 1;
			}
			...
			{
			  printf ("symbol %d has wrong size\n", inner);
			  result = 1;
			}
		    }
		}
