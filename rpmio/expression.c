...
static Value doPrimary(ParseState state)
{
  ...
  case TOK_OPEN_P:
    ...
    if (state->nextToken != TOK_CLOSE_P) {
      exprErr(state, _("unmatched ("), position);
      goto err;
    }
    if (rdToken(state))
      goto err;
  ...
  case TOK_MINUS:
    ...
    if (! valueIsInteger(v)) {
      exprErr(state, _("- only on numbers"), position);
      goto err;
    }
  ...
  while (state->nextToken == TOK_MULTIPLY
    ...
    if (valueIsInteger(v1)) {
      ...
      if ((i2 == 0) && (op == TOK_DIVIDE)) {
	    exprErr(state, _("division by zero"), position);
	    goto err;
      }
      if (op == TOK_MULTIPLY)
	...
	valueSetInteger(v1, i1 / i2);
      ...
      exprErr(state, _("* and / not supported for strings"), position);
      goto err;
    }
  }
...
static Value doAddSubtract(ParseState state)
{
  ...
  while (state->nextToken == TOK_ADD || state->nextToken == TOK_MINUS) {
	...
	valueSetInteger(v1, i1 - i2);
      ...
      if (op == TOK_MINUS) {
        exprErr(state, _("- not supported for strings"), position);
        goto err;
      }
