...
#ifdef HAVE_COPY_FILE_RANGE
next:	/* only reachable from the copy_file_range() path above */
#endif
	copied += n;
	if (notify)
	    notify(data, copied);
...
int rpmcpioWriteFile(rpmcpio_t cpio, int src_fd, off_t size)
{
    ...
    /* The header already fixed this member's length; a mismatch would emit
     * content disagreeing with it, so refuse rather than desync the archive.
     * The buffered path clamps to the same bound instead. */
    if (size < 0 || cpio->offset + size != cpio->fileend)
	return RPMERR_WRITE_FAILED;

    /* Absolute offset in the package where this file's content begins. */
    if (cpio->base < 0 || cpio->base > maxoff - cpio->offset)
	return RPMCPIO_COPY_FALLBACK;
...
int rpmcpioHeaderRead(rpmcpio_t cpio, char ** path, int * fx)
{
    ...
    if (!strncmp(CPIO_STRIPPED_MAGIC, magic,
        ...
        /* This gap precedes the content, so it must be consumed before a
