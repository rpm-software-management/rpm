...
Header-only signatures remain valid; legacy whole-package signatures cannot be
preserved and are removed, while the unsigned size and MD5 records required by
the v4 signature header are recomputed to describe the materialized
representation. The compressor and primary payload digest/size
