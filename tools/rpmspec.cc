...
#endif
    { "sources", 0, POPT_ARG_VAL, &mode, MODE_SOURCES,
	N_("list all sources from spec file(s)"), NULL },
    { "patches", 0, POPT_ARG_VAL, &mode, MODE_PATCHES,
	N_("list all patches from spec file(s)"), NULL },
    { "full", 0, POPT_ARG_NONE, &full, 0,
	N_("output full path for sources/patches"), NULL },
...
static int printSourceFilenames(rpmSpec spec, rpmSourceFlags what)
{
	...
	if (rpmSpecSrcFlags(src) & what) {
	    const char* url = rpmSpecSrcFilename(src, full);
	    if (url)
		fprintf(stdout, "%s\n", url);
	}
    }
...
static int printSources(rpmSpec spec)
{
    return printSourceFilenames(spec, RPMBUILD_ISSOURCE);
}

static int printPatches(rpmSpec spec)
{
    return printSourceFilenames(spec, RPMBUILD_ISPATCH);
}
...
int main(int argc, char *argv[])
{
    ...
    if (full && (mode != MODE_SOURCES && mode != MODE_PATCHES))
    ...
    case MODE_SOURCES: 
	ec = doSpec(optCon, printSources);
	break;
    
    case MODE_PATCHES:
	ec = doSpec(optCon, printPatches);
	break;   
